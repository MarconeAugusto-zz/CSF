...
figure(1)
plot(20*log10(abs(ganho_canal1)),'b');grid on;xlim([0 1000]);
hold on
plot(20*log10(abs(ganho_canal2)),'r')
hold on
plot(20*log10(abs(ganho_eq)),'y')
title('Sinal recebido'); ylabel('Potência (dBm)');xlabel('tempo (s)');
legend('Ganho Rx_1', 'Ganho Rx_2', 'Ganho MRC')

figure(2)
semilogy([0:15],taxa,'b', [0:15],taxa2,'r', [0:15],taxa_MRC,'y');grid on;
title('Desempenho BER X SNR'); ylabel('BER');xlabel('SNR [dB]');
legend('Rx_1', 'Rx_2', 'MRC')